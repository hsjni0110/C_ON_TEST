...
public class OrderTest extends IntegrationTest {
    ...
    void setUp() {
        ...
        돈까스 = foodRepository.save(돈까스(양식));
    ...
    void 회원이_장바구니에_음식을_담는다() {
        // given, when
        orderService.addOrderDetail(석진.getName(), 돈까스.getFoodName());

        // then
        Optional<Cart> cart = cartRepository.findById(석진.getName());
        assertThat(cart).isPresent();
        assertThat(cart.get().findOrderDetailByFoodName(돈까스.getFoodName())).isEqualTo("돈까스");
    }
}
